...
if [ -f "$PROJECT_DIR/pnpm-lock.yaml" ]; then
  if echo "$COMMAND" | grep -qE '^\s*npm\s+(install|run|exec|ci|start|test|build|add|remove|update|publish)\b'; then
...
if [ -f "$PROJECT_DIR/yarn.lock" ]; then
  if echo "$COMMAND" | grep -qE '^\s*npm\s+(install|ci|add|remove|update)\b'; then
    block "This project uses yarn (yarn.lock found). Use yarn instead of npm."
  fi
fi

# uv/poetry project — block raw pip install
if [ -f "$PROJECT_DIR/uv.lock" ] || [ -f "$PROJECT_DIR/poetry.lock" ]; then
...
if [ -f "$PROJECT_DIR/bun.lockb" ] || [ -f "$PROJECT_DIR/bun.lock" ]; then
  if echo "$COMMAND" | grep -qE '^\s*npm\s+(install|run|exec|ci|start|test|build|add|remove|update)\b'; then
