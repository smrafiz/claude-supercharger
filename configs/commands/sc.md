...
- `off` also moves Supercharger's **own** `~/.claude/agents/*.md` aside — they never run once the hooks are gone, but their descriptions are listed to the model every session (~2970 tokens). Identified against the reference copy in `~/.claude/supercharger/agents/`, so an agent you wrote is never touched; an install predating that copy moves nothing rather than guessing. The slash commands are deliberately **kept**: `/sc` is one of them, so removing them could leave you with no way to type `/sc on`, and the whole set costs ~650 tokens.
