...
5. `~/.claude/supercharger/audit/$(date -u +%Y-%m-%d).jsonl` (last 5 entries that have a `tool` field — `{tool, action}` for Bash, `{tool, file}` for Write/Edit, written by `hooks/audit-trail.sh`; skip `{hook, elapsed_ms, ts}` rows, which are hook timings) — recent tool activity
...
- **What to do** — concrete next step (e.g., "Run `bash ~/.claude/supercharger/tools/hook-toggle.sh confidence-gate off` for this session, or read the file before editing")
