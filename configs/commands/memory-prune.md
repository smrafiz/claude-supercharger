...
```bash
bash ~/.claude/supercharger/tools/memory-prune.sh
```
...
```bash
bash ~/.claude/supercharger/tools/memory-prune.sh --apply
```
...
**Other actions:**
- `bash ~/.claude/supercharger/tools/memory-prune.sh --list-archive` — show archived entries.
- `bash ~/.claude/supercharger/tools/memory-prune.sh --restore <name>` — bring one back (re-adds its index line).
