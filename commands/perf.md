...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/tools/hook-perf.sh $ARGUMENTS
```
...
- **Full profiling** (every hook fire, any bash): `touch ${CLAUDE_PLUGIN_DATA}/scope/.profiling`,
  use Claude for a session, re-run `/perf`, then `rm ${CLAUDE_PLUGIN_DATA}/scope/.profiling`.
