...
- [1.0.0] - 2026-04-30 — release: 1.0 launch. Version reset from 2.3.55. Headlines: real runtime confidence gate (leapfrogs SuperClaude's prompt-only equivalent), reflexion memory (per-project lesson capture + Jaccard recall), stack-derived standards for 8 ecosystems (React/Next.js/Vue/Svelte/Python/Go/Rust/PHP), 78 hooks across security/cost/memory/DX, 778 tests passing. README rewritten architecture-first with runtime-enforcement framing. All v2.3.x bug fixes (`.tmp` race conditions across 10 hooks, scope-guard atomic snapshot, `rm -rf` regex hardening, session_id sanitization, shell→python interpolation injection fix) carried forward.
