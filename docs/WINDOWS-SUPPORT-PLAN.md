...
| G4 | **PARTLY CLOSED v2.26.57.** The **statusLine backslash** half is fixed and CI-asserted: `lib/hooks.sh` built the path with Python's `expanduser`, and under Git Bash python3 is *Windows* python, so `settings.json` got `C:\Users\…` — a `statusLine.command` Git Bash cannot run, invisible to macOS/Linux CI because both emit forward slashes. **This contradicts §4.3's downgrade**, which is right about hook commands and wrong about this line. Remaining: the installer's `jq`/`python3` gates still `exit 1` with a POSIX-worded message. | prereqs | `install.sh:8-24` (jq gate), `:25-28` (python) | Installer: detect on Windows, guide `choco install jq` / python; keep the hard `jq` gate but with a Windows-specific message |
...
**Phase 2 status:** G6 closed; G4 half-closed (statusLine fixed v2.26.57; the installer dependency-gate messages remain).
