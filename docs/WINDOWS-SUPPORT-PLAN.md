...
| G4 | ~~**`jq` + `python3` not on Git Bash by default**~~ — **CLOSED (v2.26.57 + v2.26.58).** The **statusLine backslash** half is fixed and CI-asserted: `lib/hooks.sh` built the path with Python's `expanduser`, and under Git Bash python3 is *Windows* python, so `settings.json` got `C:\Users\…` — a `statusLine.command` Git Bash cannot run, invisible to macOS/Linux CI because both emit forward slashes. **This contradicts §4.3's downgrade**, which is right about hook commands and wrong about this line. The gates ran ~76 lines ABOVE the `detect_platform` call, and detect_platform is where the Windows python handling lives (`py` launcher → `python` → `py3` → shim, plus App-Execution-Aliases advice). So a Windows box with `py` but no `python3` died on "ERROR: python3 is required" while the fix sat unreachable in the same file. Reordered, the redundant gate deleted, and the jq message given a `winget`/`choco` arm — Git Bash has no brew/apt/dnf/pacman, so the generic line was all a Windows user ever saw. | prereqs | `install.sh:8-24` (jq gate), `:25-28` (python) | Installer: detect on Windows, guide `choco install jq` / python; keep the hard `jq` gate but with a Windows-specific message |
