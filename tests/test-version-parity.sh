...
PJ=$(python3 -c "import json;print(json.load(open('$REPO_DIR/.claude-plugin/plugin.json'))['version'])" 2>/dev/null)
eq "plugin.json .version" "$PJ"

# .claude-plugin/marketplace.json  .metadata.version  and  .plugins[0].version
MM=$(python3 -c "import json;print(json.load(open('$REPO_DIR/.claude-plugin/marketplace.json'))['metadata']['version'])" 2>/dev/null)
eq "marketplace.json .metadata.version" "$MM"
MP=$(python3 -c "import json;print(json.load(open('$REPO_DIR/.claude-plugin/marketplace.json'))['plugins'][0]['version'])" 2>/dev/null)
