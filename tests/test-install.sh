...
print(count)
")
# Full mode + developer = 86 hooks total (commit-check is opt-in, not counted here)
if [ "$HOOK_COUNT" -eq 86 ]; then
  pass
else
  fail "expected 86 hooks in full mode, got $HOOK_COUNT"
fi
teardown_test_home
...
print(count)
")
# standard maps to full = 86 hooks (with developer, commit-check is opt-in)
if [ "$HOOK_COUNT" -eq 86 ]; then
  pass
else
  fail "expected 86 hooks (standard→full), got $HOOK_COUNT"
fi
teardown_test_home
