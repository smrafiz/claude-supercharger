...
print(count)
")
# Full mode + developer = 79 hooks total (commit-check is opt-in, not counted here)
if [ "$HOOK_COUNT" -eq 79 ]; then
  pass
else
  fail "expected 79 hooks in full mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 17 ]; then
  ...
  fail "expected 17 hooks in safe mode, got $HOOK_COUNT"
fi
teardown_test_home
...
print(count)
")
# standard maps to full = 79 hooks (with developer, commit-check is opt-in)
if [ "$HOOK_COUNT" -eq 79 ]; then
  pass
else
  fail "expected 79 hooks (standard→full), got $HOOK_COUNT"
fi
teardown_test_home
