...
if [ "$HOOK_COUNT" -eq 124 ]; then
  pass
else
  fail "expected 124 hooks in full mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 34 ]; then
  pass
else
  fail "expected 34 hooks in safe mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 124 ]; then
  ...
  fail "expected 124 hooks (standard→full), got $HOOK_COUNT"
fi
teardown_test_home
