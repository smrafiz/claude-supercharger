...
if [ "$HOOK_COUNT" -eq 134 ]; then
  pass
else
  fail "expected 134 hooks in full mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 38 ]; then
  pass
else
  fail "expected 38 hooks in safe mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 134 ]; then
  ...
  fail "expected 134 hooks (standard→full), got $HOOK_COUNT"
fi
teardown_test_home
