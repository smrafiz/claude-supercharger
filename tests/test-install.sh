...
if [ "$HOOK_COUNT" -eq 126 ]; then
  pass
else
  fail "expected 126 hooks in full mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 35 ]; then
  pass
else
  fail "expected 35 hooks in safe mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 126 ]; then
  ...
  fail "expected 126 hooks (standard→full), got $HOOK_COUNT"
fi
teardown_test_home
