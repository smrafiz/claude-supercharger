...
print(count)
")
# Full mode + developer = 116 hooks total (+webfetch-egress-guard) (commit-* trio consolidated into commit-guard; +prompt-layer-inject +plugin-config-seed +readonly-guard;
...
# + plugin-config-seed + readonly-guard + webfetch-egress-guard = 29
if [ "$HOOK_COUNT" -eq 29 ]; then
  pass
else
  fail "expected 29 hooks in safe mode, got $HOOK_COUNT"
fi
teardown_test_home
...
print(count)
")
# standard maps to full = 116 hooks (+webfetch-egress-guard) (+prompt-layer-inject +plugin-config-seed; with developer, commit-check opt-in)
if [ "$HOOK_COUNT" -eq 116 ]; then
  pass
else
