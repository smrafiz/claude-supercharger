...
if [ "$HOOK_COUNT" -eq 144 ]; then
  pass
else
  fail "expected 144 hooks in full mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 40 ]; then
  pass
else
  fail "expected 40 hooks in safe mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 144 ]; then
  ...
  fail "expected 144 hooks (standard→full), got $HOOK_COUNT"
fi
teardown_test_home
