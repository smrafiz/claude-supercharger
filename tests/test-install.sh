...
if [ "$HOOK_COUNT" -eq 129 ]; then
  pass
else
  fail "expected 129 hooks in full mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 36 ]; then
  pass
else
  fail "expected 36 hooks in safe mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 129 ]; then
  ...
  fail "expected 129 hooks (standard→full), got $HOOK_COUNT"
fi
teardown_test_home
