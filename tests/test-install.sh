...
if [ "$HOOK_COUNT" -eq 73 ]; then
  pass
else
  fail "expected 73 hooks in full mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 11 ]; then
  ...
  fail "expected 11 hooks in safe mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 73 ]; then
  ...
  fail "expected 73 hooks (standard→full), got $HOOK_COUNT"
fi
teardown_test_home
