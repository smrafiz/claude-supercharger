...
if [ "$HOOK_COUNT" -eq 123 ]; then
  pass
else
  fail "expected 123 hooks in full mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 33 ]; then
  pass
else
  fail "expected 33 hooks in safe mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 123 ]; then
  ...
  fail "expected 123 hooks (standard→full), got $HOOK_COUNT"
fi
teardown_test_home
