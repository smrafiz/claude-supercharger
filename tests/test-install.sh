...
if [ "$HOOK_COUNT" -eq 46 ]; then
  pass
else
  fail "expected 40 hooks in full mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 9 ]; then
  ...
  fail "expected 9 hooks in safe mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 46 ]; then
  ...
  fail "expected 40 hooks (standard→full), got $HOOK_COUNT"
fi
teardown_test_home
