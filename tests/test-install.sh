...
print(count)
")
# Full mode + developer = 116 hooks total (+git-remote-guard) (commit-* trio consolidated into commit-guard; +prompt-layer-inject +plugin-config-seed +readonly-guard;
...
print(count)
")
# standard maps to full = 116 hooks (+git-remote-guard) (+prompt-layer-inject +plugin-config-seed; with developer, commit-check opt-in)
if [ "$HOOK_COUNT" -eq 116 ]; then
  pass
else
