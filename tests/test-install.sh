...
if [ "$HOOK_COUNT" -eq 122 ]; then
  pass
else
  fail "expected 122 hooks in full mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 32 ]; then
  pass
else
  fail "expected 32 hooks in safe mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 122 ]; then
  ...
  fail "expected 122 hooks (standard→full), got $HOOK_COUNT"
fi
teardown_test_home
