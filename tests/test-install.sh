...
if [ "$HOOK_COUNT" -eq 130 ]; then
  pass
else
  fail "expected 130 hooks in full mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 37 ]; then
  pass
else
  fail "expected 37 hooks in safe mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 130 ]; then
  ...
  fail "expected 130 hooks (standard→full), got $HOOK_COUNT"
fi
teardown_test_home
