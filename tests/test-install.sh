...
if [ "$HOOK_COUNT" -eq 120 ]; then
  pass
else
  fail "expected 120 hooks in full mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 31 ]; then
  pass
else
  fail "expected 31 hooks in safe mode, got $HOOK_COUNT"
fi
teardown_test_home
...
if [ "$HOOK_COUNT" -eq 120 ]; then
  ...
  fail "expected 120 hooks (standard→full), got $HOOK_COUNT"
fi
teardown_test_home
